#!/usr/bin/env python3
"""Build RL candidate data from sampled raw RL candidates.

RL records contain prompts, protocol constraints, execution/test metadata, and
reward specs. They do not contain assistant reference outputs.
"""
...
DEFAULT_INPUT = Path("/root/autodl-tmp/datasets/processed/raw_candidates/rl_candidates.jsonl")
DEFAULT_OUTPUT_DIR = Path("/root/autodl-tmp/datasets/processed/rl")
...
"""
    return base_record(
        ...
        reward_spec={
            ...
            "code_extractable": 0.10,
            "patch_apply": 0.25,
            "syntax_or_static_check": 0.15,
            "diff_similarity": 0.20,
            "intent_match": 0.15,
            "answer_quality": 0.05,
        },
        needs_code=True,
...
def row_to_rl(row: dict[str, Any]) -> dict[str, Any]:
...
def validate_record(record: dict[str, Any]) -> list[str]:
    ...
    for key in ["execution", "tests", "reward_spec"]:
        if key not in record:
            errors.append(f"missing_{key}")
    total_reward = sum(float(v) for v in (record.get("reward_spec") or {}).values())
    if abs(total_reward - 1.0) > 1e-6:
        errors.append(f"reward_sum_{total_reward:.4f}")
    return errors


def build_rl(args: argparse.Namespace) -> Counter:
    args.output_dir.mkdir(parents=True, exist_ok=True)
    output_path = args.output_dir / "rl_candidates.jsonl"
    rejected_path = args.output_dir / "rl_rejected.jsonl"
    counts: Counter = Counter()

    with output_path.open("w", encoding="utf-8") as out, rejected_path.open("w", encoding="utf-8") as rejected:
        for row in read_jsonl(args.input):
            try:
                record = row_to_rl(row)
    ...
    stats = {
        ...
        "notes": [
            ...
            "Required/optional tags are protocol constraints only; rollout responses are generated by the model.",
            "Only the model output inside <code> should be executed by the sandbox.",
        ],
    }
    write_json(args.output_dir / "rl_stats.json", stats)
    return counts
...
def main() -> None:
    args = parse_args()
    counts = build_rl(args)
    print(f"Wrote {counts['total']} RL candidate rows to {args.output_dir / 'rl_candidates.jsonl'}")
